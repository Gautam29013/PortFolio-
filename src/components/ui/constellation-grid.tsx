...
export default function ConstellationGrid({ className = "", isBackground = false }: ConstellationGridProps) {
    ...
    return (
        <div className={`relative w-full h-screen overflow-hidden select-none bg-slate-950 dark:bg-slate-950 light:bg-slate-50 ${isBackground ? 'absolute inset-0 z-0' : ''} ${className}`}>
            <canvas ref={canvasRef} className="absolute inset-0 block cursor-crosshair opacity-70" />

            {/* Seamless overlay title - hidden if used as background */}
            {!isBackground && (
