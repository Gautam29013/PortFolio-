...
export default function FocusText({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const characters = Array.from(
        container.querySelectorAll<HTMLElement>("[data-focus-char]"),
      );
      if (!characters.length) return;

      // Reveal the heading (starts opacity:0 to prevent FOUC)
      gsap.set(container.querySelector("[data-focus-heading]"), { opacity: 1 });
  ...
  return (
    ...
    >
      <p
        data-focus-heading
        className="m-0 w-full opacity-0"
        style={{
          fontSize: fontSize > 2 ? `clamp(2rem, ${fontSize}vw, 8rem)` : undefined,
      ...
      {showReplayButton && !scrub && (
        <button
          ...
          className="mt-4 inline-flex w-fit items-center justify-center rounded-full border border-border bg-foreground/5 px-5 py-2.5 font-mono text-[11px] uppercase tracking-[0.28em] text-foreground transition hover:border-foreground/40 hover:bg-foreground/10"
        >
          Replay animation
        </button>
      )}
